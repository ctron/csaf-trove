...
use crate::{
    ...
    scheduler::runner::revalidate_provider,
};
...
fn format_eta(eta_secs: u64) -> String {
    ...
    format!(
        "~{}",
        csaf_trove_common::format_duration_hms(duration, true)
    )
}
...
/// Re-validates all stored documents of a provider in the background (requires Bearer token).
pub async fn revalidate(
    ...
    tokio::task::spawn_blocking(move || {
        handle.block_on(async move {
            if let Err(e) = revalidate_provider(&state, &source).await {
                tracing::error!("Re-validation for {} failed: {e}", source.domain);
            }
        });
    });
